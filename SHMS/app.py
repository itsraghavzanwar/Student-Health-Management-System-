...
def home():
    if 'email' in session:
        # Check user's profession to redirect correctly
        email = session['email']
        ...
        if user:
            ...
            # Email not found anymore, session is bad
...
def doctor_info():
    if request.method == 'GET':
        ...
        # Get student's city from college table using email
        ...
        # No filtering initially, show all doctors in student's city
        cursor.execute("""
            SELECT 
...
@app.route('/your_activity',methods=['GET','POST'])
def your_activity():
    return render_template('your_activity.html')
...
def doctor_request():
    ...
    if request.method == 'GET':
        ...
        if doctor_id:
            ...
            # Get user rating average and count
            cursor.execute("""
                SELECT ROUND(AVG(doctor_rating), 1), COUNT(*) 
                FROM user_rating 
                WHERE doctor_id = %s
            """, (doctor_id,))
            user_rating = cursor.fetchone()  # (avg_rating, total_count)
            ...
            cursor.execute("""
                ...
                WHERE doctor_id = %s
            """, (doctor_id,))
            google_rating = cursor.fetchone()  # (rating, count)
        ...
        if student_id:
            ...
            if not Date:
                return render_template('doctor_request.html', error="Please Enter a Date!", doctor=doctor)
            if not Time:
                return render_template('doctor_request.html', error="Please Enter a Time!", doctor=doctor)
            if not Issue:
                return render_template('doctor_request.html', error="Please Enter an Issue!", doctor=doctor)
            ...
            return render_template('doctor_request.html', success="Request sent successfully!", doctor=doctor)
        
        return render_template('doctor_request.html', error="Student not found!", doctor=doctor)
